...
class Teams extends React.Component < {school: string, name: string, city: string, state: string} > {
  render() {
    ...
    return (
      <div className='card text-white bg-dark mb-3 col-md-3' style={maxWidth}>
        <div className="card-header"><h3>{oneTeam.school}</h3></div>
        <div className="card-body">
          <p className="card-text">Mascot: {oneTeam.name}<br></br>
          Location: {oneTeam.city}, {oneTeam.state}</p>
        </div>
      </div>
    );
  }
...
function App() {
  return (
    <div className="App">
      <h1>NCAA Team List</h1>
      <div className='row'>
        {teamData.map(oneTeam => <Teams {...oneTeam}/>)}
      </div>
    </div>
  );
}
